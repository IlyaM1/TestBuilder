...
class Ending_test_widget(QWidget):
    ...
    def init_UI(self):
        ...
        percent_result = int((self.result / self.max_result) * 100)
        self.result_progress.setValue(percent_result)
        self.result_progress.setStyleSheet(
            "QProgressBar::chunk *background-color: rgb({0}, {1}, 0)?".format(int(255 / 100 * (100 - percent_result)),
                                                                              int(255 / 100 * percent_result)).replace(
                '*', '{').replace('?', '}'))
...
if __name__ == '__main__':
    app = QApplication([])
    test_widget = Ending_test_widget(result=1, max_result=12)
    app.exec_()
